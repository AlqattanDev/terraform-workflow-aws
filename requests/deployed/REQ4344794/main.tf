...
# Generated for ticket: REQ4344794
# Requested by: ali.alqattan@bank-abc.com
# Created: 2025-12-04T04:06:55Z
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "requests/REQ4344794/terraform.tfstate"
    region         = "us-east-1"
...
resource "aws_security_group" "main" {
  name        = "my-tmxs-sg"
  description = "Security group for my-tmxs"
...
resource "aws_instance" "main" {
  ...
  monitoring = "development" == "production" ? true : false

  # User data script for initial setup
  user_data = <<-EOF
    #!/bin/bash
    yum update -y
    echo "Instance provisioned for ticket REQ4344794" > /var/log/provision.log
  EOF
