...
# Generated for ticket: REQ0010001
# Requested by: ali.alqattan@bank-abc.com
# Created: 2025-12-04T06:49:08Z
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "requests/REQ0010001/terraform.tfstate"
    region         = "us-east-1"
...
resource "aws_security_group" "main" {
  name        = "tmsx111-sg"
  description = "Security group for tmsx111"
...
resource "aws_instance" "main" {
  ...
  user_data = <<-EOF
    #!/bin/bash
    yum update -y
    echo "Instance provisioned for ticket REQ0010001" > /var/log/provision.log
  EOF
