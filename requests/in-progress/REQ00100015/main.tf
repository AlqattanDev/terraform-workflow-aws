...
# Generated for ticket: REQ00100015
# Requested by: ali.alqattan@bank-abc.com
# Created: 2025-12-04T06:50:56Z
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "requests/REQ00100015/terraform.tfstate"
    region         = "us-east-1"
...
resource "aws_security_group" "main" {
  name        = "tmsx1117-sg"
  description = "Security group for tmsx1117"
...
resource "aws_instance" "main" {
  ...
  user_data = <<-EOF
    #!/bin/bash
    yum update -y
    echo "Instance provisioned for ticket REQ00100015" > /var/log/provision.log
  EOF
